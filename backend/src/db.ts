...
<blockquote>📊 Успех в крипте = умение отличать реальные проекты от rug pulls. Учитесь анализировать!</blockquote>`
  ...
  for (const course of courses.rows) {
    // Match by first word to handle Unicode differences
    const courseFirstWord = course.title.split(' ')[0];
    const courseDataItem = courseData.find(c => c.title.split(' ')[0] === courseFirstWord);
    
    if (!courseDataItem) {
      // Add default modules
      await pool.query(`INSERT INTO course_modules (course_id, title, content, order_index) VALUES ($1, $2, $3, $4)`, 
        [course.id, '📖 Введение', '<h2>Добро пожаловать!</h2><p>Контент скоро появится.</p>', 1]);
      await pool.query(`INSERT INTO course_modules (course_id, title, content, order_index) VALUES ($1, $2, $3, $4)`, 
        [course.id, '📖 Основная часть', '<h2>Продолжение следует...</h2><p>Контент скоро появится.</p>', 2]);
      await pool.query(`INSERT INTO course_modules (course_id, title, content, order_index) VALUES ($1, $2, $3, $4)`, 
        [course.id, '📖 Заключение', '<h2>Финал!</h2><p>Контент скоро появится.</p>', 3]);
      continue;
    }
